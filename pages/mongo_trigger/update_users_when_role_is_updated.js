exports = async function(changeEvent) {
	...
	*/
	...
	if(changeEvent.operationType === 'update') {
        ...
        // New Role
		const fullDocument = changeEvent.fullDocument;
		console.log(`Role updated: ${fullDocument.name}`);

        // Looking for uses that have this role
        ...
        // Update users
		console.log(`Users qty: ${users.length}`);
		users.forEach(async user => {

			console.log(`User to Update: ${user.username}`);

            // New role's lists
			const roles = user.roles.map(role => {
				if (role._id.toString() === changeEvent.documentKey._id.toString()) {
					return fullDocument;
				}
				return role;
			});

			// Replace all roles of the user
			const db_resp = await db.collection("users").updateOne(
				{
					_id: user._id
				},
				{
